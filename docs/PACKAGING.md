...
当前稳定版本是 `v0.11.1`；OpenWrt APK、Arch `pkgver`、Git tag、Linux 与 macOS 构件均使用 `0.11.1`。
...
`v0.11.1` 面向 OpenWrt 25.12.5 x86/64：

| 包 | 版本 | 所有内容 |
|---|---|---|
| `steer` | `0.11.1-r1` | 控制器、默认 UCI、procd init、完整只读 SRS seed |
| `luci-app-steer` | `0.11.1-r1` | LuCI 页面、ucode RPC、ACL |
| `luci-i18n-steer-zh-cn` | `0.11.1-r1` | 简体中文翻译 |
...
Swift 测试、构建和输出路径查询统一显式使用 `--build-system native`。Swift 6.4 的默认 Swift Build 在当前工具链中会把 Mach-O 的 SDK 版本写成部署版本 13.0；原生构建器经验证正确记录 SDK 27.0，验收仍同时要求最低部署版本 13.0 和 SDK 27。
