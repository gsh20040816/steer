	...
	contract := Contract{
		...
		IDPolicy: IDPolicy{
			...
			CollectionPrefixes: map[string]string{
				"wireguard_tunnels": "wg", "nodes": "node", "routes": "route", "dns_profiles": "dns", "local_proxies": "proxy",
				"rules": "rule", "subscriptions": "subscription",
			},
		},
		CreationDefaults: map[string]map[string]interface{}{
			"wireguard_tunnels": {"enabled": true, "address": []string{}, "private_key": "", "peers": []any{}, "access": []any{}},
		...
		CreationRequiredFields: map[string][]string{
			"wireguard_tunnels": {"id", "enabled", "address", "private_key", "peers"},
		...
		CollectionOrdering: map[string]CollectionOrderingPolicy{
			"wireguard_tunnels": {StableIDField: "id", MoveActions: []string{"up", "down"}},
			"nodes": {
			...
			"routes":        {StableIDField: "id", MoveActions: []string{"up", "down"}, MovableKinds: []string{"single", "wireguard"}},
		...
		RouteKinds:          choices("direct", "Direct", "block", "Reject", "single", "Single node", "wireguard", "WireGuard"),
		...
		RuleMatchFields:          []string{"allowed_ips", "inbound", "domain_match", "ip_match", "source_ip_cidr", "source_mac_address", "network", "protocol", "port"},
		RuleConnectionOnlyFields: []string{"allowed_ips", "ip_match", "network", "protocol", "port"},
